  ...
  trigger_api:
    ...
    steps:
      ...
      - name: Authenticate with Keycloak
        id: keycloak_auth
        run: |
          TOKEN_RESPONSE=$(curl -s -X POST 'https://auth.prod-azdo.br.ai-impact.gft-cloud.com/realms/ai-impact/protocol/openid-connect/token' \
            -H 'Content-Type: application/x-www-form-urlencoded' \
            -d 'client_id=ai-impact-client' \
            -d "username=jaqueline" \
            -d "password=12345" \
            -d "scope=openid" \
      ...
      - name: Trigger Code Review API
        ...
        run: |
          ...
          RESPONSE=$(curl --location 'https://api.prod-azdo.br.ai-impact.gft-cloud.com/ai/review' \
      ...
      - name: Monitor Code Review Job Status
        ...
        run: |
          ...
          while [[ "$STATUS" != "Completed" ]]; do
            RESPONSE=$(curl --location "https://api.prod-azdo.br.ai-impact.gft-cloud.com/ai/jobs/$JOB_ID/status" \
              --header "Authorization: Bearer ${{ env.access_token }}")
